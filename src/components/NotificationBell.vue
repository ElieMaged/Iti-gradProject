  ...
  <div class="notification-container">
    ...
    <div v-if="showNotifications" class="notifications-dropdown">
      <div class="notifications-header">
        <h3>Notifications</h3>
        <button @click="markAllAsRead" class="mark-all-read purple">
          Mark all as read
        </button>
      </div>
        ...
        >
          <div class="notification-icon">
            <i
              ...
              v-if="notification.type === 'payment_received'"
            ></i>
            <i
              class="fas fa-calendar-plus"
              v-else-if="notification.type === 'booking_request'"
            ></i>
            <i
            ...
            <i
              ...
              v-else-if="notification.type === 'technician_application'"
            ></i>
            <i
              class="fas fa-check-circle"
              v-else-if="
      ...
      <div class="notifications-footer">
        ...
        <button
          ...
          class="test-notification-btn purple"
        >
          Clear All
        </button>
      </div>
...
async function clearAllNotifications() {
  ...
  try {
    ...
    alert(
      `✅ Successfully deleted ${notifications.value.length} notifications.`
    );
...
onMounted(async () => {
  ...
  const unsubscribeAuth = onAuthStateChanged(auth, async (user) => {
    ...
    try {
      ...
      if (
        user.email === "elie1400674@gmail.com" ||
        user.email === "tasneemmostafa200110@gmail.com"
      ) {
      ...
      const unsubscribeNotifications = onSnapshot(
        ...
        (error) => {
          ...
          if (
            userError.code === "failed-precondition" ||
            userError.message.includes("index")
          ) {
            ...
            const unsubscribeFallbackUser = onSnapshot(
              fallbackUserQuery,
              (fallbackUserSnapshot) => {
                console.log(
                  "✅ Fallback user notification snapshot received:",
                  fallbackUserSnapshot.docs.length,
                  "notifications"
                );

                const unsubscribeFallbackAdmin = onSnapshot(
                  fallbackAdminQuery,
                  (fallbackAdminSnapshot) => {
                    console.log(
                      "✅ Fallback admin notification snapshot received:",
                      fallbackAdminSnapshot.docs.length,
                      "notifications"
                    );

                    const fallbackUserNotifications =
                      fallbackUserSnapshot.docs.map((doc) => ({
                        id: doc.id,
                        ...doc.data(),
                      }));

                    const fallbackAdminNotifications =
                      fallbackAdminSnapshot.docs.map((doc) => ({
                        id: doc.id,
                        ...doc.data(),
                      }));

                    // Combine and remove duplicates
                    const allFallbackNotifications = [
                      ...fallbackUserNotifications,
                      ...fallbackAdminNotifications,
                    ];
                    const uniqueFallbackNotifications =
                      allFallbackNotifications.filter(
                        (notification, index, self) =>
                          index ===
                          self.findIndex((n) => n.id === notification.id)
                      );
                    ...
                    console.log(
                      ...
                      uniqueFallbackNotifications
                    );
                    notifications.value = uniqueFallbackNotifications;
                  },
                  (fallbackAdminError) => {
                    console.error(
                      "❌ Fallback admin query also failed:",
                      fallbackAdminError
                    );
                  }
                );
              ...
              (fallbackUserError) => {
                console.error(
                  "❌ Fallback user query also failed:",
                  fallbackUserError
                );
              }
            );
