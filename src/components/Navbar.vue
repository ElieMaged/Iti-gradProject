...
<template>
  <!-- Top Bar -->
  <div class="w-full hidden sm:block" id="contact-Nav">
    <div class="flex justify-between items-center py-2 h-14 mx-20">
      <div class="flex items-center gap-6">
        <a
          href="tel:+12095551234"
          class="contact-links no-underline flex items-center gap-2 text-sm"
          ><i class="fa-solid fa-phone"></i>{{ $t("contactPhone") }}</a
        >
        <a
          :href="`mailto:boltfix@gmail.com`"
          class="contact-links no-underline flex items-center gap-2 text-sm"
          ><i class="fa-solid fa-envelope"></i>boltfix@gmail.com</a
        >
        <a
      ...
      <div class="flex gap-2">
        <a
          href="#"
          class="flex items-center justify-center rounded-full social-media-links w-7 h-7"
          ><i class="fa-brands fa-facebook-f secondary-color"></i
        ></a>
        <a
          href="#"
          class="flex items-center justify-center rounded-full social-media-links w-7 h-7"
          ><i class="fa-brands fa-x-twitter secondary-color"></i
        ></a>
        <a
          href="#"
          class="flex items-center justify-center rounded-full social-media-links w-7 h-7"
          ><i class="fa-brands fa-linkedin-in secondary-color"></i
        ></a>
        <a
          href="#"
          class="flex items-center justify-center rounded-full social-media-links w-7 h-7"
  ...
  <!-- Navbar -->
  <nav
    class="mx-20 navbar py-3 flex justify-between items-center relative gap-3.5"
  >
    <!-- Logo -->
    <div class="flex items-center img-container">
      <img
        src="/logo/ace04d3b268cf810c91d002fdf7454a6ef778f27.png"
        alt="Logo"
        class="h-full w-full"
        id="logo"
      />
    ...
    <button
      class="md:hidden block text-2xl"
    ...
    <ul class="hidden md:flex gap-8 font-medium m-0">
      <li>
        <router-link
          :to="locale === 'ar' ? getRoute('') : '/'"
          class="no-underline services-color"
          >{{ $t("navHome") }}</router-link
        >
      </li>
      <li>
        <router-link
          :to="locale === 'ar' ? getRoute('about') : '/about'"
          class="no-underline services-color"
      ...
      <li>
        <router-link
          :to="locale === 'ar' ? getRoute('contact') : '/contact'"
          class="no-underline services-color"
      ...
      >
        <span
          class="no-underline services-color cursor-pointer flex items-center"
        ...
        <div
          :class="[
            'services-dropdown',
            isMobile
              ? 'mt-2 relative w-full left-0'
              : 'absolute !-left-24 mt-2 shadow-lg rounded z-50',
          ]"
          v-show="true"
          @mouseenter="!isMobile && showDropdown"
        >
          <router-link to="/allservices">{{ $t("allServices") }}</router-link>
          ...
          <router-link to="/elecTechnicians">{{
    ...
    <div class="hidden md:flex items-center gap-2">
      ...
      <template v-else>
        <template v-if="user">
          <span
            class="flex items-center gap-2 rounded bg-gray-100 cursor-pointer"
            @click="handleProfileClick"
          >
            <span class="text-gray-700 font-semibold px-3 py-1">
              {{ user.email || user.uid }}
              <span class="text-xs text-gray-500 ml-2"
                >({{ getRoleDisplayText() }})</span
              >
            </span>
            <i class="fa-regular fa-user fa-xl user-icon"></i>
          </span>
    ...
    <transition name="fade">
      ...
      >
        <div
          class="md:hidden absolute inset-0 overlay"
          @click="closeMobileMenu"
        ></div>
        <div
          class="md:hidden fixed top-0 left-0 w-4/5 max-w-xs h-full shadow-lg z-50 p-4 overflow-y-auto flex flex-col"
        >
          ...
          >
            ...
            <i
              :class="
                navAccordionOpen ? 'fa fa-chevron-up' : 'fa fa-chevron-down'
              "
            ></i>
          </button>
          <div v-if="navAccordionOpen" class="pl-2 pb-2">
            <router-link
              :to="locale === 'ar' ? getRoute('') : '/'"
              class="block py-2 no-underline services-color"
              @click="closeMobileMenu"
              >{{ $t("navHome") }}</router-link
            >
            <router-link
            ...
            >
              ...
              <div
                :class="[
                  ...
                  isMobile
                    ? 'mt-2 relative w-full left-0'
                    : 'absolute !-left-20 mt-2 shadow-lg rounded z-50',
                ]"
                v-show="true"
                @mouseenter="!isMobile && showDropdown"
              >
                <router-link to="/allservices" @click="closeMobileMenu">{{
                  $t("allServices")
                }}</router-link>
                <router-link to="/plumbing" @click="closeMobileMenu">{{
                  $t("navPlumbing")
                }}</router-link>
                <router-link to="/aircondition" @click="closeMobileMenu">{{
                  $t("navAirConditioner")
                }}</router-link>
                <router-link to="/electricity" @click="closeMobileMenu">{{
                  $t("navElectricity")
                }}</router-link>
                <router-link to="/wallfinishing" @click="closeMobileMenu">{{
                  $t("navWallFinishing")
                }}</router-link>
                <router-link to="/carpentry" @click="closeMobileMenu">{{
                  $t("navCarpentry")
                }}</router-link>
                <router-link to="/elecTechnicians" @click="closeMobileMenu">{{
                  $t("navElectricityTechnicians")
                }}</router-link>
              </div>
            </div>
          ...
          <div class="mt-4 flex flex-col gap-2">
            ...
            <template v-else>
              <template v-if="user">
                <span class="flex items-center gap-2">
                  <span
                    class="text-gray-700 font-semibold px-3 py-1 rounded bg-gray-100 cursor-pointer"
                    @click="handleProfileClick"
                  >
                    {{ user.email || user.uid }}
                    <span class="text-xs text-gray-500 ml-2"
                  ...
                  <i
                    class="fas fa-user-circle text-2xl text-secondary cursor-pointer"
                    @click="handleProfileClick"
                  ></i>
                </span>
                <button
            ...
            <NotificationBell class="mobile-notification-bell" />
          </div>
        </div>
      </div>
    </transition>
...
.services-dropdown {
  ...
  box-shadow: 0 8px 24px rgba(0, 0, 0, 0.1), 0 1.5px 4px rgba(0, 0, 0, 0.05);
  padding: 0.5rem 0;
  z-index: 50;
  opacity: 0;
  pointer-events: none;
